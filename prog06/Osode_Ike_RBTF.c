 ...
 * Ike Osode
 * proj# 6
 * This file uses different functions to perform a variety of different
 * computations on a binary tree. In order to use it, one must input a list
 * of operations in the command line which will be read by the main function
 * and used to call the appropriate functions.
****** */
#include <stdio.h>
#include <stdbool.h>
#include <stdlib.h>
...
int main(int argc, char * argv[]) {
  ...
  for(int i = 1; i < argc; i++) {
      if (strcmp(argv[i], "isordered") == 0) {
            // Check if the tree is heap-ordered
            ...
            // Parse the target value for the contains check
...
/* ******
 * create and initialize a tree Node with the given data
 * Parameters: int theData
 * return new Node
****** */
...
bool isHeapOrdered( Node *curr )
{
  ...
  return isHeapOrdered(curr->right) && isHeapOrdered(curr->left);  
}
...
int numSingleChild ( Node *curr )
{
    ...
    return count + numSingleChild(curr->left) + numSingleChild(curr->right);
}
 ...
 * Parameter: Node *curr
 * return int num of nodes in subtree
****** */
int size( Node *curr )
{
  if ( curr == NULL ) {
 ...
 * Parameters: Node *curr, int target
 * return boolean
****** */
bool contains( Node *curr, int target )
{
...
/* ******
 * This function frees the Node n.
 * Parameter: Node *n
 * return void
****** */
...
/* ******
 * Function that inserts child node in subtree
 * Parameter: Node * n, int parentData, char branch, int childData
 * return boolean
****** */
...
/* ******
 * Function that frees space of binary tree
 * Parameters: BinaryTree *bt
 * return void
****** */
...
/* ******
 * Function that inserts ChildBinaryTree
 * Parameters: BinaryTree * bt, int parentData, char branch, int childData
 * return boolean
****** */
 ...
 * Parameters: BinaryTree * bt, int exitCode
 * return void
****** */
